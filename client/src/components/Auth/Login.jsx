...
const Login = () => {
  ...
  const handleLogin = async (formData) => {
    ...
    try {
      ...
      // ✅ FIX: Match HTML version response structure
      if (response.success) {
        setSuccessMessage('Login successful! Redirecting...');
        
        // ✅ FIX: Store exactly like HTML version
        localStorage.setItem('auth_token', response.data.token); // Use auth_token not authToken
        localStorage.setItem('auth_user', JSON.stringify(response.data.user)); // Use auth_user not authUser
        
        // ✅ FIX: Redirect to connections page
        setTimeout(() => {
          window.location.href = '/connections';
        }, 800); // Match HTML timing (800ms)
      } else {
        // ✅ FIX: Use response.message like HTML version
        setErrorMessage(response.message || 'Invalid credentials');
      }
    } catch (error) {
